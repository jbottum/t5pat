...
In our simple tests with our older MacAir, we saw that the model can take 6-8 minutes to run to completion.  The Large model runs faster and starts to produce output in 1-2 minutes.   The XL model runs after the Large model and the XL can take 4-5 minutes to complete.   You should expect a delay between the end of the Large and the start of output of the XL and you will receive a status message during this time, i.e. "loading checkpoint shards".   Your script's processing time will vary on your computer's configuration and its load.  
